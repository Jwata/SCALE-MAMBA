...
stop_players
docker container rm ${DOCKER_DUMMY} > /dev/null || true
docker volume rm ${VOL_CERTS} ${VOL_PROGS} ${VOL_DATA} ${VOL_TESTDATA} > /dev/null || true
docker network rm ${DOCKER_NET} > /dev/null || true
